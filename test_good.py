...
class ImageClassifier():
    ...
    def build_del_map(self):
        ...
        #self.del_map = np.ones(self.del_map.shape)
    ...
    def classify_sliced (self, img, slice_map):
        ...
        for i in range(slice_map.shape[0]):
            for j in range(slice_map.shape[1]):
                if slice_map[i,j] == 0:
                    masked_middle[0,i,j,:] = self.del_map[0,i,j,:]
    ...
    def infer_map_build (self, middle, slice_map, dynamic_window=True):
        ...
        if dynamic_window:
            ...
            return self.infer_map_build_single(middle, slice_map)
    ...
    def slice_minimum_slow (self, middle, dynamic_window=True):
    ...
    def slice_minimum (self, img, times=None):
        infer_map_list = []
        slice_map_list = []
        slice_map = np.ones(self.m.shape[1:3])
        ...
        #for tm in range(times):
            ...
            if second:
                ...
                infer_map = self.infer_map_build(self.m, slice_map, dynamic_window=False)
            ...
            for i,j in min_indices:
                ...
                ind += 1
                if self.classify_middle_sliced(slice_map) != self.type_pred:
...
def main():
    ...
    #0117Ae
    """
    inf, draw = image_classifier.slice_minimum(img)
    #it = ti
    fig = plt.figure()
    it = len(inf)
    ...
    for i in range(it):
        print(i,'th slicing')
        image_classifier.classify_sliced_loud(img, draw[i])
